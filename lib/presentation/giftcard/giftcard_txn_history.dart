...
class GiftcardTxnHistoryView extends StatefulWidget {
  const GiftcardTxnHistoryView({super.key});

  @override
  State<GiftcardTxnHistoryView> createState() => _GiftcardTxnHistoryViewState();
}

class _GiftcardTxnHistoryViewState extends State<GiftcardTxnHistoryView> {
  @override
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      TxnHistoryProvider txnHistoryProvider =
          Provider.of<TxnHistoryProvider>(context, listen: false);
      txnHistoryProvider.updateGiftcardStats().then((_) {}).catchError((_) {});
      await fetchHistory();
    });
  ...
  // ALL
  int all_txn_page = 1;
  List<GiftcardTxnHistory> transactionHistory = [];
  List<GiftcardTxnHistory> allBought = [];
  List<GiftcardTxnHistory> allSold = [];

  // FILTER
  int filter_txn_page = 1;
  List<GiftcardTxnHistory> allFilter = [];
  List<GiftcardTxnHistory> boughtFilter = [];
  List<GiftcardTxnHistory> soldFilter = [];

  // SEARCH
  int search_txn_page = 1;
  List<GiftcardTxnHistory> allSearch = [];
  List<GiftcardTxnHistory> boughtSearch = [];
  List<GiftcardTxnHistory> soldSearch = [];
  ...
  Widget build(BuildContext context) {
    ...
    return CustomScaffold(
      ...
      appBar: CustomAppBar(
        title: Text("Giftcard Transaction History", style: get20TextStyle()),
      ),
      body: Padding(
              ...
              children: [
                Expanded(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: ColorManager.kGiftcardTab1,
                    ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          "${txnHistoryProvider.getGiftCardPartailAndApprovedSoldCount()}",
                          ...
                          text: TextSpan(
                            ...
                            children: <TextSpan>[
                              TextSpan(
                                text: formatNumber(
                                  txnHistoryProvider
                                      .getGiftCardPartailAndApprovedSoldAmount(),
                                ),
                              ),
                              const TextSpan(
                ...
                Expanded(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(4),
                      color: ColorManager.kGiftcardTab2,
                    ),
                    child: Column(
                      mainAxisSize: MainAxisSize.max,
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        Text("Giftcards Bought:",
                            style: get12TextStyle()
                                .copyWith(color: ColorManager.kSecBlue)),
                        const SizedBox(height: 8),
                        Text(
                          "${txnHistoryProvider.getGiftCardPartailAndApprovedBuyCount()}",
                          style: get24TextStyle().copyWith(
                            fontWeight: FontWeight.w500,
                            color: ColorManager.kSecBlue,
                          ),
                          ...
                          text: TextSpan(
                            ...
                            children: <TextSpan>[
                              TextSpan(
                                text: formatNumber(
                                  txnHistoryProvider
                                      .getGiftCardPartailAndApprovedBuyAmount(),
                                ),
                              ),
                              const TextSpan(
                                  text: ' NGN', style: TextStyle(fontSize: 16)),
                ...
                : (getElements().isEmpty)
                    ...
                    : Expanded(
                        child: RefreshIndicator(
                          ...
                          child: GroupedListView<GiftcardTxnHistory, String>(
  ...
  List<GiftcardTxnHistory> getElements() {
  ...
  Future<void> fetchHistory() async {
    ...
    String query = "?per_page=100&include=bank,giftcardProduct";
    ...
    await TransactionHelper.getGiftcardTxnHistory(query).then((value) {
      if (value.isNotEmpty) {
        if (filterQueryParam != null) {
